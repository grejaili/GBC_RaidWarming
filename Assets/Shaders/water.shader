    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float4 node_6610 = _Time + _TimeEditor;
                float2 node_4815 = (i.uv0+node_6610.g*float2(0.02,0.02));
...
////// Specular:
                ...
                float2 node_5300 = (float2(i.uv0.r,(i.uv0.g+_WaterFoamV))+node_6610.g*float2(0,0.07));
...
/// Final Color:
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3x3 tangentTransform = float3x3( i.tangentDir, i.bitangentDir, i.normalDir);
                float3 viewDirection = normalize(_WorldSpaceCameraPos.xyz - i.posWorld.xyz);
                float4 node_3340 = _Time + _TimeEditor;
                float2 node_4815 = (i.uv0+node_3340.g*float2(0.02,0.02));
                float4 _Refraction_var = tex2D(_Refraction,TRANSFORM_TEX(node_4815, _Refraction));
                float3 normalLocal = normalize(lerp(float3(0,0,1),_Refraction_var.rgb,_Reflection));
                float3 normalDirection = normalize(mul( normalLocal, tangentTransform )); // Perturbed normals
                float2 sceneUVs = float2(1,grabSign)*i.screenPos.xy*0.5+0.5 + (_Refraction_var.rgb.rg*(_Distortion*0.2));
...
////// Specular:
                ...
                float3 specularColor = 0.0;
                float specularMonochrome;
                float2 node_5300 = (float2(i.uv0.r,(i.uv0.g+_WaterFoamV))+node_3340.g*float2(0,0.07));
                float4 _Foam_var = tex2D(_Foam,TRANSFORM_TEX(node_5300, _Foam));
                float node_1150 = (_Foam_var.r*i.vertexColor.a);
                float3 diffuseColor = ((_Color.rgb*i.vertexColor.a)+(node_1150-0.1)); // Need this for specular when using metallic
                diffuseColor = DiffuseAndSpecularFromMetallic( diffuseColor, specularColor, specularColor, specularMonochrome );
